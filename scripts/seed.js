...
async function seed() {
  ...
  try {
    ...
    // Seed admin (upsert - won't overwrite existing)
    const adminUsername = process.env.ADMIN_USERNAME || 'aaddy307';
    const adminPasswordHash = process.env.ADMIN_PASSWORD_HASH
      ? process.env.ADMIN_PASSWORD_HASH
      : bcrypt.hashSync('Mh05fl7946', 10);

    const existingAdmin = await Admin.findOne({ username: adminUsername });
    if (existingAdmin) {
      console.log(`ℹ️ Admin "${adminUsername}" already exists, skipping`);
    } else {
      await Admin.create({
